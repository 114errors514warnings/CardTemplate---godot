...
public partial class AddState : BaseButtonCommand
{
	protected override string ParameterFormat => "目标UniqueInGameId 状态ID [层数]";

	public override void _Ready()
	{
		Pressed += OnAddStatePressed;
	}

	private void OnAddStatePressed()
	{
		...
		if (battleSytem == null)
		{
			AppendConsoleError("错误：未找到 BattleSytem 节点，无法增加状态。");
			return;
		}

		if (string.IsNullOrEmpty(raw))
		{
			AppendConsoleInfo($"增加状态 参数格式：{ParameterFormat}");
		...
		int stacks = 1;
		if (args.Length >= 3 && !int.TryParse(args[2], out stacks))
		{
			AppendConsoleError($"错误：层数 '{args[2]}' 不是合法数字。参数格式：{ParameterFormat}");
			return;
		}

		if (!battleSytem.TryAddStateToUnit(targetUniqueInGameId, rawStateType, stacks, out string resultMessage))
		{
