...
public partial class LoadCardCsv : Node
{
	...
	/// CSV表头常量
	/// </summary>
	public static readonly string CSV_HEADER = "CardId,UniqueInGameId,EnergyCost,Category,EffectType,EffectDescription,NeedTarget,ExtraAttack,ExtraShield";

	/// <summary>
	/// 从CSV文件加载所有卡牌
	/// CSV格式: CardId,UniqueInGameId,EnergyCost,Category,EffectType,EffectDescription,NeedTarget,ExtraAttack,ExtraShield
	/// </summary>
	...
	private static Card ParseCardFromCSVLine(string line)
	{
		try
		{
			...
			string uniqueInGameId = fields[1];
			int energyCost = int.Parse(fields[2]);
			string categoryStr = fields[3];
			string effectTypeStr = fields[4];
			string effectDescription = fields[5];
			bool needTarget = bool.Parse(fields[6]);
			...
			// 根据类型创建相应的卡牌对象
			return CreateCardByCategory(category, cardId, uniqueInGameId, energyCost, effectType, effectDescription, needTarget, fields);
		}
		catch (Exception ex)
		{
	...
	/// 根据卡牌类别创建相应的卡牌对象
	/// </summary>
	private static Card CreateCardByCategory(CardCategory category, int cardId, string uniqueInGameId, int energyCost, EffectType effectType, string effectDescription, bool needTarget, string[] fields)
	{
		switch (category)
		{
			case CardCategory.Attack:
				...
				return new AttackCard(cardId, uniqueInGameId, energyCost, effectType, effectDescription, needTarget, extraAttack, extraShield);

			case CardCategory.Skill:
				int skillExtraShield = fields.Length > 8 ? int.Parse(fields[8]) : 0;
				return new SkillCard(cardId, uniqueInGameId, energyCost, effectType, effectDescription, needTarget, skillExtraShield);

			case CardCategory.State:
				return new StateCard(cardId, uniqueInGameId, energyCost, effectType, effectDescription, needTarget);
	...
	private static string CardToCSVLine(Card card)
	{
		...
		return $"{card.CardId},{card.UniqueInGameId},{card.EnergyCost},{card.Category},{card.EffectType},{LoadCsv.EscapeCSVField(card.EffectDescription)},{card.NeedTarget},{extraAttack},{extraShield}";
	}
